	  ...
	  /*   if (Tnow==0)
	     	...
	     	
	     {
	    	 ...
	    	 for (int i=0;i<StaticParametre.nbrapps_parallel;i++)// placement des taches initiales,selon le nombre de cluster ///
	    		...
	    		//ajouter cette application à la liste des apps en cours d'exécution
	    		 PlacementStatic(i); 
		 ...
		 for (int i=0;i<StaticParametre.nbrapps_parallel;i++)// placement des taches initiales,selon le nombre de cluster ///
 	 	{
			...
			if(!StaticParametre.applicationAttente.isEmpty())
			{
	    	 ...
	    	 // Placement des taches non initiales qui demandent a etre placé, en parcourant la list des taches par application en cours d'execution
	    	 					...
	    	 					
	    	 				{
	    	 					...
	    	 					tache=StaticParametre.application_en_cours[j].getListTache().get(i);
	    			 	    			 		...
	    			 	    			 		for(int k=0;k<List_fils.size();k++){
	    			 	    			 			...
	    			 	    			 			// lancement du placement du fils avec comme parametres: la tache fils à placer et le processeur sur le quel la tache pere est placé
	    			 	    			 				new Methode_Placement(tache_fils,Create_NOC.getNOC()[tache.x][tache.y]);
	    			 	    			 				
	    			 	    			 				
	    			 	    			 				
	    			 	    			 				
	    			 	    			 				if(tache_fils.debut_execution!=-1)  // Fils placé 
	    			 	    			 				{
...
////////////	 methode qui permet de placer statiquement une application au milieu d un cluster ////
	  ...
	  public static void PlacementStatic(int id_cluster) throws InterruptedException 
	  {
		  ...
		  StaticParametre.listClusters[id_cluster].Free=false;// rendre le cluster  occupé
	    	
 		 StaticParametre.applicationAttente.getFirst().setIdCluster(id_cluster);//définir le cluster de l'appplication
 		...
 		tacheInit= StaticParametre.applicationAttente.getFirst().getListTache().get(0);
 		 ...
 		 if (StaticParametre.MONO_MULTI.equals("MONO"))
 			 processeurInit.set_state(false);// MONO, rendre le processeur occupé
 		
 		 else
 			 processeurInit.setMem(tacheInit.getTailleTache( processeurInit.getType()),1);//MULTI; décrementer le mémoire du proc
 		 ...
 		 processeurInit.ajoutTache(tacheInit);// ajouter la tache initiale sur le proc adéquat
 		 
 		
 		 //ajouter le temps de fin d'execution de la tache à la liste des evenements
...
////////////////	methode qui est appelée lorsque une application a fini de s'executer //////////  
