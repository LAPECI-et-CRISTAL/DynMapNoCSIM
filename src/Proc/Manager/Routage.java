...
public class Routage {
  ...
  {
		  for(int j=0;j<StaticParametre.nbrapps_parallel;j++)// parcour la list des tache qui nécissent la communication
		 	{
		 		int c;
		 				...
		 				for(int i=0;i<StaticParametre.application_en_cours[j].getListTache().size();i++)
		 				{
		 					...
		 					
		 					{
		 						...
		 						{// lancer le routage entre le pere et le fils
	 								...
	 								Create_NOC.getNOC()[tache_fils.x][tache_fils.y].nbr_paquet_reçu.add(0);
	 								...
	 								for(int r=1;r<=tache_pere.getDonneePartager(k)/StaticParametre.DEBIT;r++)
		 							{// creation de paquets !!
