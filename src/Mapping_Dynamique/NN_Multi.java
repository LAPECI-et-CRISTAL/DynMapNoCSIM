...
// verifier si la tâche a été placé ou non 
  ...
  if(placé==false)
  {
	...
	while(placé==false && i<=StaticParametre.Limit_NOC_x-1)
	{
	 // temps_recherche=temps_recherche+100;
	   ...
	   i++; 
	  refaire(tachePlacer,getX(),getY()-i); // refaire la recherche à partir du proc gauche
	  

	 
	  if(placé==false)		  
			  
      refaire(tachePlacer,getX()+i,getY()); //refaire la recherche à partir du proc bas
	  
	  
	  
	  
	  if(placé==false)
	      refaire(tachePlacer,getX()-i,getY()); //refaire la recherche à partir du proc haut
	  
	  
	if (placé==false)
		  refaire(tachePlacer,getX(),getY()+i); //refaire la recherche à partir du proc droit
	
	if(placé==false && i==StaticParametre.Limit_NOC_x-1) {
								int c=0;
								placé=true;
...
placé=false;																																																																			//if (StaticParametre.f.equals("scenario1.xml")) { this.=this.+3500;this.Energie=this.Energie+380;}	
...
//////////////// verifier si la tache a été placé lors de la premiere exécution du Spiral 
		...
		if(placé==false)
		{
			...
			{
				i++;
			refaire_Spiral(tachePlacer,this.x,this.y-i);//reprendre la recherche à partir du processeur gauche
			
			if(placé==false)
					refaire_Spiral(tachePlacer,this.x+i,this.y);//reprencdre la recherche à partir du processeur bas
				
			
			 if(placé==false) 
                	 refaire_Spiral(tachePlacer,this.x-i,this.y);//reprendre la recherche à partir du processeur haut
             
			
			 
			 if(placé==false)
  				refaire_Spiral(tachePlacer,this.x,this.y+i);//reprencdre la recherche à partir du processeur droit
			
			 
			if(placé==false)
				refaire_Spiral(tachePlacer,this.x-i,this.y-i);//reprendre la recherche à partir du processeur haut gauche
				
			
                
 			
                 		if(placé==false)
                      			refaire_Spiral(tachePlacer,this.x-i,this.y+i);//reprencdre la recherche à partir du processeur haut droit
                 		  
                 			
                 		if(placé==false)
								refaire_Spiral(tachePlacer,this.x+i,this.y-i);//reprencdre la recherche à partir du processeur bas gauche
		
                 		
                 					if(placé==false)
                      						refaire_Spiral(tachePlacer,this.x+i,this.y+i);//reprencdre la recherche à partir du processeur bas droit
