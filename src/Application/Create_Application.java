...
/**
 * Cette classe permet de charger les applications à partir d'un fichier de spécification xml donné en entrer 
 * 
 * 
 *
 */
...
public class Create_Application {
	...
	public static void remplirTaches(String lienFichier){
		...
		try {
			...
			//récupérer la racine <list_applicatio>
			Element racine = document.getRootElement();
		
			//récupérer la list des balise <application>
						...
						for(int j=0; j<list_succ.size(); j++){
							...
							if(!list_id.isEmpty()){
								...
								for(int k=0; k<list_id.size(); k++){
									...
									// définir l'identificateur du fils
									list.add(Integer.parseInt(c.getAttributeValue("id")));
									
									// données partagées avec chaque fils
